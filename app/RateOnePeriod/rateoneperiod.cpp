...
int main(int argc, char *argv[]) {
  ...
  LIF lif(options.get_parameter_file());
  CosineSignal signal(options.get_parameter_file());
...
#pragma omp parallel for
  ...
  for (int i = 0; i < time.get_steps(); i++) {
    rate_one_period[i % period] +=
        (double)(period) / (double)time.get_steps() * rate[i];
  };
  ...
  for (int i = 0; i < period; i++) {
    t += time.get_dt();
    std::cout << t << " " << (double)(1.0 / N) * rate_one_period[i]
              << std::endl;
  };
