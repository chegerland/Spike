...
int main(int argc, char *argv[]) {
  ...
  const TimeFrame time_frame_short(time_frame.get_t_0() + 100.,
                                   time_frame.get_t_end(), time_frame.get_dt());
  const LIFAC neuron(input_file);

  // get array for wiener kernel
  double *kernel;
  kernel = new double[time_frame_short.get_steps()];
  ...
  // dummies
  const int offset =
      (int)(time_frame.get_steps() - time_frame_short.get_steps());
  const double T = time_frame_short.get_t_end() - time_frame_short.get_t_0();
...
#pragma omp parallel for
  for (int i = 0; i < N_sims; i++) {
    ...
    //fill wiener kernel, begin after certain offset
    // loop over spikes
    //for (int j = 0; j < time_frame.get_steps(); j++) {
    //  // if neuron has spiked at index j...
    //  if (spike_train.get_spike(j)) {
    //    // calculate kernel with lag times up to the offset
    //    for (int k = 0; k < j - offset; k++) {
    //      kernel[k] += 1. / ((double)N_sims * T) * signal.get_value(j - k);
    //    }
    //  }
    //}
    neuron.get_kernel(time_frame, signal, norm, offset, kernel);
...
#pragma omp critical
  ...
  fftw_plan p;
  p = fftw_plan_dft_r2c_1d((int)time_frame_short.get_steps(), kernel, suscept,
                           FFTW_ESTIMATE);
  fftw_execute(p);
  fftw_destroy_plan(p);
  ...
  double scale = time_frame_short.get_dt();
  for (unsigned int i = 0; i < time_frame_short.get_steps(); i++) {
    file
    //<< (double)i / (time_frame.get_t_end() - time_frame.get_t_0()) << ","
   //      << scale * suscept[i][0] << "," << scale * suscept[i][1] << ","
         << time_frame_short.get_time(i) - 100 << "," << kernel[i] << "\n";
  }
